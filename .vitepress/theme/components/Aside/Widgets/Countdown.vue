<template>
  ...
  <div class="count-down s-card">
    <div class="count-left">
      ...
      <span class="time"> {{ daysUntil }} </span>
...
const dateList = computed(() => {
  const raw = theme.value?.aside?.countDown?.data?.date;
  if (Array.isArray(raw)) return raw;
  return raw ? [raw] : [];
});

const formatCountdownDate = (value) => {
...
const dateLabel = computed(() => dateList.value.map(formatCountdownDate).filter(Boolean).join(" / "));
const daysUntil = computed(() => getDaysUntil(dateList.value));
