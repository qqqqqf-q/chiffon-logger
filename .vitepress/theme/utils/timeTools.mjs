...
export const getDaysUntil = (dateStr) => {
  ...
  const dateList = Array.isArray(dateStr) ? dateStr : [dateStr];
  ...
  const normalizeInput = (input) => {
    ...
    if (/^\d{4}-\d{2}-\d{2}$/.test(normalized)) return dayjs(normalized);
    if (/^\d{2}-\d{2}$/.test(normalized)) return dayjs(`${base.year()}-${normalized}`);
    if (/^\d{4}$/.test(normalized)) {
      return dayjs(`${base.year()}-${normalized.slice(0, 2)}-${normalized.slice(2)}`);
    }
  ...
  const candidates = dateList
    ...
    .filter(Boolean)
    .map((item) => {
      const target = item.startOf("day");
      return target.isBefore(base, "day") ? target.add(1, "year") : target;
    });

  if (candidates.length === 0) return 0;
  return Math.min(...candidates.map((item) => item.diff(base, "day")));
};
